...
The [`practice`](practice/) folder includes dialect-specific database work:

- [`01_DDL_DML_Pt1.sql`](practice/01_DDL_DML_Pt1.sql): DuckDB schema, table, insert, update, delete, and cleanup practice
- [`02_database_lifecycle_mysql.sql`](practice/02_database_lifecycle_mysql.sql): MySQL `DROP DATABASE`, `CREATE DATABASE`, `SHOW DATABASES`, `USE`, and schema lifecycle commands

The MySQL file is intentionally separate because DuckDB cannot execute MySQL's database-level `CREATE DATABASE` and `DROP DATABASE` statements.
